...
const CustomerInfoForm: React.FC<CustomerInfoFormProps> = ({ translations, onComplete, onBack, userId, keyParam }) => {
  ...
  const resetForm = () => {
    ...
    const kycInput = document.getElementById('kycFile-input') as HTMLInputElement;
    const idCardInput = document.getElementById('idCardFile-input') as HTMLInputElement;
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div>
            ...
            <Input id="nickname" name="nickname" type="text" value={formData.nickname} onChange={handleChange} placeholder={translations.nicknamePlaceholder} required={false} disabled={isSubmitting} />
          </div>

          <div>
            <label htmlFor="phone" className="block mb-2 text-sm font-medium text-gray-600 dark:text-gray-300">
        ...
        <div>
          ...
          <Input id="email" name="email" type="email" value={formData.email} onChange={handleChange} placeholder={translations.emailPlaceholder} required={false} disabled={isSubmitting} />
        </div>

   <div>
          <label className="block mb-2 text-sm font-medium text-gray-600 dark:text-gray-300">{translations.idCardFileLabel || 'Upload ID Card Photo'}</label>
          <FileUpload id="idCardFile-input" name="idCardFile" buttonText={translations.idCardFileButtonText || 'Choose ID Card File'} noFileText={translations.noFileChosen} onChange={handleFileChange} file={formData.idCardFile} disabled={isSubmitting} />
        </div>
        <div>
        ...
        <div className="mb-4">
          <p className="mb-2 text-sm font-medium text-gray-700 dark:text-gray-300">{translations.kycGuideText || 'How to take a proper KYC photo:'}</p>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="border rounded-lg p-3 bg-green-50 dark:bg-green-500/10">
              <p className="text-green-700 dark:text-green-300 text-sm font-semibold mb-1">✔ {translations.do}</p>
              <img src="https://storage.googleapis.com/my-kyc-images/do.jpg" alt="KYC Do" className="rounded-md shadow-sm w-full" />
            </div>
            <div className="border rounded-lg p-3 bg-red-50 dark:bg-red-500/10">
              <p className="text-red-700 dark:text-red-300 text-sm font-semibold mb-1">✖ {translations.dont}</p>
              <img src="https://storage.googleapis.com/my-kyc-images/dont.jpg" alt="KYC Don't" className="rounded-md shadow-sm w-full" />
            </div>
          </div>
        </div>
        ...
        <div className="pt-2 min-h-[60px]">
          {submissionStatus === 'success' && (
            <div className="text-center p-3 rounded-md bg-green-100 dark:bg-green-500/20 text-green-700 dark:text-green-300">{translations.submittedMessage}</div>
          )}
          {submissionStatus === 'error' && (
            <div className="text-center p-3 rounded-md bg-red-100 dark:bg-red-500/20 text-red-700 dark:text-red-300">{submissionError}</div>
          )}
